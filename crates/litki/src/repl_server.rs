...
use bevy::log;
use jinme::prelude::EvalContext;
use jinme::symbol::Symbol;
...
impl ReplServer {
            ...
            match line_result {
                Ok(line) => {
                    ...
                    if line == "(help)" {
                        ...
                        let _ = writeln!(writer, "  (litki.commands/command <args>) - Execute a registered command");
                        let _ = writeln!(writer, "  e.g.:");
                        let _ = writeln!(writer, "  (litki.commands/echo :hi)");
                        let _ = writeln!(writer, "  (litki.commands/stable-ids)");
                        let _ = writeln!(writer, "  (litki.commands/get-template-names)");
                        let _ = writeln!(writer, "  (litki.commands/register-template :deer [[:litki.vital/health {{:max 150}}]])");
                        let _ = writeln!(writer, "  (litki.commands/spawn :deer)");
                        let _ = writeln!(writer, "  (litki.commands/inflict-damage :deer 10)");
                        let _ = writeln!(writer, "  (litki.commands/query-entities-with-health)");
                        let _ = writer.flush();
                        continue;
                    }
    ...
    fn dispatch_command(
        ...
        if let Some(list) = value_optics::preview_list(&value) {
            if let Some(head) = list_optics::view_first_as_symbol(&list) {
                let command_name = match head {
                    Symbol::Unqualified(head) => head.name().to_owned(),
                    Symbol::Qualified(head) => format!("{}/{}", head.namespace(), head.name()),
                };
        ...
        let evaled = jinme::core::eval(env, EvalContext::default(), value);
        Ok(format!("{evaled}"))
    }
            ...
            loop {
                ...
                if handle.is_finished() {
                    match handle.join() {
                        ...
                        Err(_) => {
                            log::info!("REPL server thread panicked");
    ...
    fn drop(&mut self) {
        ...
        if let Some(handle) = self.handle.take() {
            ...
            while start.elapsed() < timeout {
                if handle.is_finished() {
                    if let Ok(_result) = handle.join() {
                        log::info!("REPL server thread exited cleanly");
                    } else {
                        log::info!("REPL server thread panicked on drop");
                    }
                    return;
                }
                thread::sleep(Duration::from_millis(100));
            ...
            log::info!("REPL server accept thread did not exit within {} seconds", timeout.as_secs());
        }
    }
}
