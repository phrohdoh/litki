...
use bevy::log;
use jinme::{prelude::{BufReadHandle, Environment, EvalContext, FunctionArity, IFunction, List, Map, Namespace, PtrNamespace, PtrValue, Symbol, SymbolUnqualified, Value, Var, Vector, WriteHandle, apply, closure_fn, eval, try_resolve, value}, environment::PtrEnvironment, handle::Handle};
use crate::script::CommandPromise;
...
pub fn create_env() -> PtrEnvironment {
    let env = {
    ...
    clojure_core.build_and_bind_function(
        "-",
        vec![
            closure_fn(FunctionArity::Exactly(1), |_env: PtrEnvironment, _ctx: EvalContext, args: Vec<PtrValue>| {
                let any_arg_is_float = args.iter().map(Arc::as_ref).any(Value::is_float);
                if any_arg_is_float {
                    let mut x = 0f64;
                    for arg in args {
    ...
    clojure_core.build_and_bind_function(
        "map",
        vec![
            closure_fn(FunctionArity::Exactly(2), |env: PtrEnvironment, ctx: EvalContext, args: Vec<PtrValue>| {
                let f = args[0].clone();
                let coll = args[1].clone();
                match coll.as_ref() {
                    Value::List(list, _) => Value::new_list_ptr(list.iter().map(|element| apply(env.clone(), ctx.clone(), f.clone(), vec![element.to_owned()])).collect()),
                    Value::Vector(vector, _) => Value::new_vector_ptr(vector.iter().map(|element| apply(env.clone(), ctx.clone(), f.clone(), vec![element.to_owned()])).collect()),
                    Value::Set(set, _) => Value::new_set_ptr(set.iter().map(|element| apply(env.clone(), ctx.clone(), f.clone(), vec![element.to_owned()])).collect()),
                    Value::Map(map, _) => Value::new_vector_ptr(map.iter().map(|(k, v)| {
                        let new_kv = apply(env.clone(), ctx.clone(), f.clone(), vec![
    ...
    clojure_core.build_and_bind_function(
        "prn",
        vec![
            closure_fn(FunctionArity::AtLeast(0), |env: PtrEnvironment, _ctx: EvalContext, args: Vec<PtrValue>| {
    ...
    clojure_core.build_and_bind_function(
        ...
        vec![
            ...
            }), closure_fn(FunctionArity::Exactly(2), |_env: PtrEnvironment, _ctx: EvalContext, args: Vec<PtrValue>| {
    ...
    clojure_core.build_and_bind_function(
        "resolve",
        vec![
            closure_fn(FunctionArity::Exactly(1), |env: PtrEnvironment, _ctx: EvalContext, args: Vec<PtrValue>| {
    ...
    clojure_core.build_and_bind_function(
        ...
        vec![
            ...
            closure_fn(FunctionArity::Exactly(3), |_env: PtrEnvironment, _ctx: EvalContext, args: Vec<PtrValue>| {
    ...
    clojure_core.build_and_bind_function(
        "eval",
        vec![
            closure_fn(FunctionArity::Exactly(1), |env: PtrEnvironment, ctx: EvalContext, args: Vec<PtrValue>| eval(env, ctx, args[0].clone()))],
    );
    ...
    clojure_core.build_and_bind_function(
        "apply",
        vec![
            closure_fn(FunctionArity::AtLeast(1), |env: PtrEnvironment, ctx: EvalContext, args: Vec<PtrValue>| apply(env, ctx, args[0].clone(), args[1..].to_vec()))],
    );

    clojure_core.build_and_bind_function(
        "list",
        vec![
            closure_fn(FunctionArity::AtLeast(0), |_: PtrEnvironment, _ctx: EvalContext, args: Vec<PtrValue>| Value::new_list_ptr(args))],
    );
    ...
    clojure_core.build_and_bind_function(
        "all-ns",
        vec![
            closure_fn(FunctionArity::Exactly(0), |env: PtrEnvironment, _ctx: EvalContext, _args: Vec<PtrValue>| {
                Value::new_list_ptr(
                    env.all_namespaces()
                        .into_iter()
    ...
    clojure_core.build_and_bind_function(
        "in-ns",
        vec![
            closure_fn(FunctionArity::Exactly(1), |env: PtrEnvironment, _ctx: EvalContext, args: Vec<PtrValue>| -> PtrValue {
    ...
    clojure_core.build_and_bind_function(
        "ns-name",
        vec![
            closure_fn(FunctionArity::Exactly(1), |_env: PtrEnvironment, _ctx: EvalContext, args: Vec<PtrValue>| -> PtrValue {
    ...
    clojure_core.build_and_bind_function(
        "ns-resolve",
        vec![
            closure_fn(FunctionArity::Exactly(2), |env: PtrEnvironment, _ctx: EvalContext, args: Vec<PtrValue>| -> PtrValue {
                let ns = args[0].try_get_handle::<PtrNamespace>()
    ...
    clojure_core.build_and_bind_function(
        "ns-unalias",
        vec![
            closure_fn(FunctionArity::Exactly(2), |_env: PtrEnvironment, _ctx: EvalContext, args: Vec<PtrValue>| -> PtrValue {
                let ns = args[0].try_get_handle::<PtrNamespace>()
    ...
    clojure_core.build_and_bind_function(
        ...
        vec![
            closure_fn(FunctionArity::Exactly(2), |env: PtrEnvironment, _ctx: EvalContext, args: Vec<PtrValue>| -> PtrValue {
                ...
                Arc::new(Value::var(var))
            }), closure_fn(FunctionArity::Exactly(3), |env: PtrEnvironment, _ctx: EvalContext, args: Vec<PtrValue>| -> PtrValue {
                let ns = match args[0].try_get_handle::<PtrNamespace>() {
                    Ok(ns) => ns,
                    Err(_) => {
                        let sym = value::optics::preview_symbol(args[0].as_ref())
    ...
    clojure_core.build_and_bind_function(
        "meta",
        vec![
            closure_fn(FunctionArity::Exactly(1), |_env, _ctx: EvalContext, args: Vec<_>| {
    ...
    clojure_core.build_and_bind_function(
        "with-meta",
        vec![
            closure_fn(FunctionArity::Exactly(2), |_env, _ctx: EvalContext, args: Vec<PtrValue>| {
    ...
    clojure_core.build_and_bind_function(
        "get",
        vec![closure_fn(FunctionArity::Exactly(2), |env: PtrEnvironment, ctx: EvalContext, args: Vec<PtrValue>| {
            let mut args = args.into_iter();
            ...
            env.get_namespace_or_panic("clojure.core")
                ...
                .invoke(env.clone(), ctx.clone(), vec![m, k, d])
        }), closure_fn(FunctionArity::Exactly(3), |_env: PtrEnvironment, _ctx: EvalContext, args: Vec<PtrValue>| {
            let mut args = args.into_iter();
            let m = args.next().unwrap();
            if m.is_nil() { return m; }
    ...
    clojure_core.build_and_bind_function(
        ...
        vec![(
            closure_fn(FunctionArity::Exactly(2), |env: PtrEnvironment, ctx: EvalContext, args: Vec<PtrValue>| {
                ...
                let mut v = m;
                for k in ks {
                    v = get_fn.invoke(env.clone(), ctx.clone(), vec![v, k]);
    ...
    clojure_core.build_and_bind_function(
        "assoc",
        vec![
            closure_fn(FunctionArity::AtLeast(3), |_env: PtrEnvironment, _ctx: EvalContext, args: Vec<PtrValue>| {
...
/// - (litki.commands/cmd-name arg1 arg2)           → blocks indefinitely
/// - (litki.commands/cmd-name arg1 arg2 :timeout 5000 :default result)  → timeout in ms
pub fn bind_execute_command_function(
    ...
    clojure_core.build_and_bind_function(
        "!",
        vec![
            closure_fn(FunctionArity::AtLeast(1), move |_env: PtrEnvironment, _ctx: EvalContext, args: Vec<PtrValue>| {
                if args.is_empty() {
